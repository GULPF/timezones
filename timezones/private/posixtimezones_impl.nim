import std / [options, os, strutils, algorithm, sugar]
import zone1970, coordinates, .. / .. / timezones

# TODO: It would simplify a lot if we can convert this to using
# `timezonefile.Transition` instead.
...
proc loadLocation(tzPath: string, dir: string):
    Option[(Coordinates, seq[Country])] =
...
proc loadTzImpl*(path, dir: string, loadLocation = false):
    tuple[
      transitions: seq[Transition],
      leapSeconds: seq[LeapSecond],
      stdName: string,
  ...
  let optDstName = if hasDST: some(dstName) else: none(string)
  result.transitions = transitions
  result.leapSeconds = leapSeconds
  result.stdName = stdName
  result.dstName = optDstName

  if loadLocation:
    let opt = loadLocation(path, dir)
    if opt.isSome:
      result.location = some(opt.get[0])
      result.countries = opt.get[1]